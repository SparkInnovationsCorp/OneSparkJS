((global) => {
     ...
     OneSparkJs.Inputs = (() => {
          ...
          const mouseCanvasEnterEvent = (event) => {
               handleGenericEvent(event, EventTypes.MOUSE_CANVAS_ENTER);
          };

          const mouseCanvasLeaveEvent = (event) => {
               handleGenericEvent(event, EventTypes.MOUSE_CANVAS_LEAVE);
          };
          ...
          const handleGenericEvent = (event, eventType) => {
          ...
          const handleMouseEvent = (event, eventType) => {
               ...
               for (let i = 0; i < handlers[eventType].length; i++) {
                    ...
                    if (handler.region) {
                         ...
                         )
                         {
                              if (eventType == EventTypes.MOUSE_MOVE && (handler.isOver)) {
                                   ...
                                   handleGenericEvent(event, EventTypes.MOUSE_LEAVE);
                              }

                              if (eventType == EventTypes.MOUSE_CLICK) {
                                   handler.isOver = false;
                                   handleGenericEvent(event, EventTypes.MOUSE_CLICK_OUT);
                              }
                    ...
                    if (eventType == EventTypes.MOUSE_MOVE && (!handler.isOver)) {
                         handler.isOver = true;
                         handleGenericEvent(event, EventTypes.MOUSE_ENTER);
                    }
               ...
               for (let i = 0; i < handlers[eventType].length; i++) {
                    ...
                    if (handler.region) {
                         if (
                              ...
                              if (eventType == EventTypes.TAP) {
                                   handler.isOver = false;
                                   handleGenericEvent(event, EventTypes.TAP_OUT);
                              }
